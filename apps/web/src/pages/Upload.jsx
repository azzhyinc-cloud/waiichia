...
function FAlbum({ok,uf,sL,sE,sM,sP,sS,tk}){
  ...
  const submit=async()=>{
    ...
    try{
      ...
      const r=await fetch(API+'/api/albums',{
        ...
        body:JSON.stringify({
          title:f.title,description:f.desc,genre:f.genre,
          country:f.country,release_year:parseInt(f.year)||2026,
          album_type:f.type,cover_url:cu||null,is_published:true,tags:[]
        })
      })
      ...
      for(let i=0;i<tracksWithFile.length;i++){
        const t=tracksWithFile[i]
        sM(`Upload piste ${i+1}/${tracksWithFile.length}: ${t.title}`)
        const au=await uf(t.file,'/api/upload/audio')
        const tr=await fetch(API+'/api/tracks/',{
          method:'POST',
          headers:{'Content-Type':'application/json','Authorization':'Bearer '+tk},
          body:JSON.stringify({title:t.title,type:'music',genre:f.genre,country:f.country,audio_url_128:au,cover_url:cu||null,is_published:true,access_type:'free',tags:[]})
        })
        const td=await tr.json()
        if(tr.ok&&td.track?.id){
          await fetch(API+'/api/albums/'+albumId+'/tracks',{
            method:'POST',
            headers:{'Content-Type':'application/json','Authorization':'Bearer '+tk},
            body:JSON.stringify({track_id:td.track.id,position:i+1})
          }).catch(()=>{})
        }
      }
      sS(true)
  ...
  return(<div className="upload-form-panel">
    ...
    <div className="form-group"><label className="label">🖼️ Pochette *</label>
      <div className="cover-upload-row">
        <div className="cover-preview" onClick={()=>document.getElementById('cA').click()}>
          <input type="file" id="cA" accept="image/*" style={{display:'none'}} onChange={e=>{const x=e.target.files[0];if(x){setCf(x);setCp(URL.createObjectURL(x))}}}/>
          {cp?<img src={cp} alt=""/>:<><span>+</span><div style={{fontSize:10,marginTop:4}}>Ajouter</div></>}
        </div>
        <div style={{fontSize:12,color:'var(--text2)'}}>Carré · 1000×1000px min</div>
      </div>
    </div>
    <div className="form-row">
      <div className="form-group" style={{flex:2}}><label className="label">Titre *</label><input className="input-field" value={f.title} onChange={e=>s('title',e.target.value)} placeholder="Titre de l'album..."/></div>
      <div className="form-group"><label className="label">Type</label><select className="select-styled" style={{width:'100%'}} value={f.type} onChange={e=>s('type',e.target.value)}><option value="album">Album</option><option value="ep">EP</option><option value="single">Single</option><option value="mixtape">Mixtape</option></select></div>
    </div>
    <div className="form-row" style={{gridTemplateColumns:'1fr 1fr 1fr'}}>
      <div className="form-group"><label className="label">Genre</label><select className="select-styled" style={{width:'100%'}} value={f.genre} onChange={e=>s('genre',e.target.value)}>{GENRES_MUSIC.map(g=><option key={g}>{g}</option>)}</select></div>
      <div className="form-group"><label className="label">Pays</label><select className="select-styled" style={{width:'100%'}} value={f.country} onChange={e=>s('country',e.target.value)}>{COUNTRIES.map(([c,l])=><option key={c} value={c}>{l}</option>)}</select></div>
      <div className="form-group"><label className="label">Année</label><input className="input-field" type="number" value={f.year} onChange={e=>s('year',e.target.value)}/></div>
    </div>
    ...
    <div className="upload-section-box">
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
        <div className="upload-section-title" style={{marginBottom:0}}>🎵 Pistes</div>
        <button className="btn btn-outline btn-sm" onClick={()=>setTks(p=>[...p,{title:'',file:null}])}>+ Ajouter</button>
      </div>
      {tks.map((_,i)=>(
        <div key={i} className="album-track-row">
          <div className="track-row-num">{i+1}</div>
          <div className="track-row-body">
            <input className="input-field" placeholder="Titre piste *" style={{marginBottom:8}} value={tks[i].title} onChange={e=>setTks(p=>p.map((t,j)=>j===i?{...t,title:e.target.value}:t))}/>
            <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
              <label className="upload-mini-btn" style={{cursor:'pointer'}}>
                🎵 {tks[i].file?tks[i].file.name.slice(0,20)+'…':'Audio'}
                <input type="file" accept="audio/*" style={{display:'none'}} onChange={e=>{const x=e.target.files[0];if(x)setTks(p=>p.map((t,j)=>j===i?{...t,file:x}:t))}}/>
              </label>
            </div>
          </div>
          {tks.length>1&&<button className="track-row-remove" onClick={()=>setTks(p=>p.filter((_,j)=>j!==i))}>✕</button>}
        </div>
      ))}
