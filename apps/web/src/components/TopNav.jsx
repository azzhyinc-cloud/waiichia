...
export default function TopNav() {
  ...
  useEffect(()=>{
    ...
    setShowRes(true)
    // Tenter API
    api.tracks.search?.(`?q=${encodeURIComponent(search)}&limit=5`)
      .then(d=>{
        if(d.tracks?.length){
          const apiRes=d.tracks.map(t=>({type:"track",title:t.title,sub:t.profiles?.display_name||"Artiste",icon:"🎵",id:t.id}))
          setResults(apiRes)
        }
      }).catch(()=>{})
  },[search])

  // Fermer search au click extérieur
  useEffect(()=>{
  ...
  return (
    <>
      <div style={{
        ...
        zIndex:150,transition:"background var(--transition)",
        ...
        <div ref={searchRef} style={{flex:1,maxWidth:460,position:"relative"}}>
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)}
            onFocus={()=>search&&setShowRes(true)}
          ...
          {showRes&&results.length>0&&(
            <div style={{position:"absolute",top:"calc(100% + 8px)",left:0,right:0,
              ...
              boxShadow:"0 8px 32px rgba(0,0,0,.3)",zIndex:300,overflow:"hidden"}}>
              {results.map((r,i)=>(
                <div key={i}
                  onClick={()=>{setSearch("");setShowRes(false);r.type==="artist"?setPage("profile",{profileUsername:r.title.toLowerCase().replace(" ","")}):null}}
                  style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",cursor:"pointer",
                    borderBottom:i<results.length-1?"1px solid var(--border2)":"none",transition:"background .15s"}}
                  onMouseEnter={e=>e.currentTarget.style.background="var(--card)"}
                  onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                  <div style={{width:32,height:32,borderRadius:r.type==="artist"?"50%":8,
              ...
              <div style={{padding:"8px 14px",borderTop:"1px solid var(--border)",fontSize:11,color:"var(--text3)",textAlign:"center",cursor:"pointer",fontFamily:"Plus Jakarta Sans,sans-serif"}}
                onClick={()=>{setShowRes(false)}}>
                Appuyez sur Entrée pour rechercher "{search}"
              </div>
            </div>
          )}
        </div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,marginLeft:"auto",flexShrink:0}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6,padding:"7px 12px",background:"var(--card)",border:"1px solid var(--border)",borderRadius:50,cursor:"pointer",fontSize:12,fontFamily:"Plus Jakarta Sans,sans-serif",transition:"border-color .2s",color:"var(--text2)"}}
